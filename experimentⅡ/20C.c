...
while(1){
	...
	if(course == 100){
		...
		course = 102;
		rotate_tire( CW, CW, 1, 1, 20, 75*0.086*ONE_ROT);	
		goto LOOP1;
	}else if(course == 102){
		course = 103;
		...
		course = 104;
		rotate_tire( CW, CW, 1, 1, 20, 37.5*0.075*ONE_ROT);
		goto LOOP1;
	}else if(course == 104){
		course = 105;
		...
		course = 108;
		rotate_tire( CW, CW, 1, 1, 20, 75*0.075*ONE_ROT);
		goto LOOP1;
	}else if(course == 108){
		course = 109;
		rotate_tire( CW, CW, 0, 2, 40, ROT_90);	// 左へ90度回転する
...
//----赤外線センサ------//
		...
		if(g_Motor_Distance != 0 ){
				...
				change_motor_l_speed( CW, 20 );			// 左モータをDuty20%で前転させる
				...
				change_motor_r_speed( CW, 20 );			// 右モータをDuty20%で前転させる
				//右横、左横に障害物有り	
